...
export function Leaderboard({revision}:{revision:number}){
 ...
 return <section className="leaderboard" id="leaderboard" aria-labelledby="leaderboard-title">
  <div className="leaderboard-heading"><div><p className="eyebrow">TODAY // UTC PRIZE ROUND</p><h2 id="leaderboard-title">TODAY'S FASTEST FINISHES</h2></div><div className="leaderboard-actions"><ShareRound/><button onClick={()=>setRetry(x=>x+1)}>REFRESH</button></div></div>
  <div className="daily-pot" aria-label="Daily prize distribution"><div><strong>DAILY TRADING FEE POT</strong><span>Awarded every day at 00:00 UTC</span></div><ol>{medals.map((medal,i)=><li className={medal.tone} key={medal.tone}><span className={`doom-medal ${medal.tone}`} aria-hidden="true"><img src="/doom-exe-hell-icon.png" alt=""/></span><b>{i+1}{i===0?'ST':i===1?'ND':'RD'}</b><strong>{medal.share}</strong></li>)}</ol></div>
  <p className="ranking-note">Reach the exit, enter your name, and post your time. Today's three fastest finishers share the pot: 1st wins 70%, 2nd wins 20%, and 3rd wins 10%. Rankings reset at 00:00 UTC.</p>
  {loading?<p role="status">Loading rankings…</p>:error?<div className="ranking-error" role="alert">{error} <button onClick={()=>setRetry(x=>x+1)}>TRY AGAIN</button></div>:entries.length===0?<div className="ranking-empty">NO FINISHERS YET.<span>Be the first to make it out.</span></div>:<div className="ranking-scroll"><table><thead><tr><th scope="col">RANK</th><th scope="col">PLAYER</th><th scope="col">ROUND TIME</th><th scope="col"><span className="sr-only">Share round</span></th></tr></thead><tbody>{entries.map((entry,i)=>{const medal=medals[i];return <tr className={medal?'podium-row '+medal.tone:''} key={`${entry.finishedAt}-${i}`}><td><span className="rank-number">{String(i+1).padStart(2,'0')}</span>{medal&&<span className={`doom-medal ${medal.tone}`} title={medal.label}><img src="/doom-exe-hell-icon.png" alt=""/><span className="sr-only">{medal.label}</span></span>}</td><td>{entry.name}</td><td className="round-time"><time>{formatTime(entry.elapsedMs)}</time></td><td><ShareRound elapsedMs={entry.elapsedMs} name={entry.name} compact/></td></tr>})}</tbody></table></div>}
 </section>;
}
