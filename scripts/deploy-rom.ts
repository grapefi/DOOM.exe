...
const meta=JSON.parse(await readFile('web/public/rom/manifest.json','utf8'));validateMeta(meta);
const bytes=new Uint8Array(await readFile('web/public/rom/rom.bin'));checkHash(bytes,meta.compressedHash,meta.compressedSize);await decompressVerified(bytes,meta);
...
if(!/^0x[0-9a-fA-F]{64}$/.test(process.env.PRIVATE_KEY??'')) throw new Error('Set a testnet PRIVATE_KEY in .env');
...
if(await client.getChainId()!==chain.id) throw new Error('Refusing to deploy outside chain 421614');
...
async function deploy(name:string,args:unknown[]) {
 ...
 if(await client.getBalance({address:account.address})<ceiling) throw new Error('Insufficient testnet ETH');
