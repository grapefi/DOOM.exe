...
export function PrizePool(){
 ...
 useEffect(()=>{
  ...
  async function load(){if(busy)return;busy=true;try{const next=await scoreApi<PoolData>('/api/prize-pool',undefined,controller.signal);if(!controller.signal.aborted){setData(next);setError('');}}catch(e){if(!controller.signal.aborted)setError(e instanceof Error?e.message:'Prize pool unavailable.');}finally{busy=false;}}
 ...
 return <section className="prize-pool" aria-labelledby="prize-pool-title">
  ...
  <div className="prize-status"><div>{error?<span role="alert">{data?'Showing the last confirmed balance. ':''}{error} <button onClick={()=>setRetry(x=>x+1)}>RETRY</button></span>:data?<span>Balance checked {new Date(data.checkedAt).toLocaleTimeString()} · updates every minute{data.tokens==='0'?' · awaiting funding':''}</span>:<span role="status">Checking the prize wallet…</span>}</div>
   ...
   {data?.quoteAt&&<small>USD estimate uses Robinhood’s bid/ask midpoint and share multiplier. Quote: {new Date(data.quoteAt).toLocaleString()}.</small>}
  </div>
 </section>;
}
