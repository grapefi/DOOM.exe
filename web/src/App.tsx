  ...
  <section className="console">
   <div className="screen-column" ref={viewport}>
    ...
    <div className="viewport">
     ...
     {(phase==='ready'||phase==='error')&&<div className="boot"><p className="eyebrow">// {source==='arbitrum'?'ARBITRUM PLAYTHROUGH':'ROBINHOOD MAINNET'} //</p><img className="game-brand" src="/doom-exe-hell-icon.png" alt="DOOM.EXE"/><p className="game-slogan">HELL HAS A NEW HOME.</p><h2>ENTER THE<br/><em>EXECUTION ZONE</em></h2><p>Survive the crowd. Scavenge supplies. Reach the exit.</p><button className="primary" onClick={launch} disabled={source==='robinhood'&&!manifestAddress}>{source==='arbitrum'?'▶ PLAY ARBITRUM VERSION':manifestAddress?'▶ PLAY ON ROBINHOOD CHAIN':'ROBINHOOD CHAIN · DEPLOYING SOON'}</button><small>{source==='arbitrum'?'On-chain Arbitrum preview · available for recording · no wallet needed':manifestAddress?'On-chain cartridge · no wallet needed':'Select Arbitrum Playthrough to record the game now.'}</small></div>}
     {phase==='loading'&&<div className="boot loading"><img className="game-brand compact" src="/doom-exe-hell-icon.png" alt="DOOM.EXE"/><p className="eyebrow">BOOT SEQUENCE</p><h2>LOADING<span className="blink">_</span></h2><progress value={progress.percent} max={100}/><p role="status">{progress.label}</p><button className="text-button" onClick={reset}>CANCEL</button></div>}
     {phase==='dead'&&<div className="finished death-screen" role="region" aria-label="You died"><img className="game-brand compact" src="/doom-exe-hell-icon.png" alt="DOOM.EXE"/><p className="eyebrow">ROUND OVER</p><h2>YOU DIED.</h2><p>The arena claimed another.</p><p className="result-time">{formatTime(elapsed)}</p><button className="primary play-again" onClick={launch}>↻ PLAY AGAIN</button></div>}
     {phase==='complete'&&<div className="finished" role="region" aria-label="Round completed"><img className="game-brand compact" src="/doom-exe-hell-icon.png" alt="DOOM.EXE"/><p className="eyebrow">EXIT REACHED</p><h2>EXECUTED.</h2><p className="result-time">{formatTime(elapsed)}</p>
