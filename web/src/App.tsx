...
export default function App(){
 ...
 return <main>
  <header className="site-hero">
   <div className="hero-art"><img className="hero-image" src="/doom-chain-header.png" alt="DOOM CHAIN — Hell has a new home. Play on Robinhood Chain."/></div>
   <div className="brand-bar"><a className="brand" href="/" aria-label="DOOM CHAIN home"><img className="header-wordmark" src="/doom-chain-wordmark.png" alt="DOOM CHAIN"/></a><div className="network"><i/> ROBINHOOD CHAIN <b>MAINNET / {chain.id}</b></div><div className="header-actions"><a className="competition-link" href="#competition">PRIZE + LEADERBOARD ↓</a><a className="github-link" href="https://github.com/grapefi/DOOM.exe" target="_blank" rel="noreferrer">GITHUB ↗</a></div></div>
  </header>
  ...
  <section className="console">
   <div className="screen-column" ref={viewport}>
    ...
    <div className="viewport">
     ...
     {(phase==='ready'||phase==='error')&&<div className="boot"><p className="eyebrow">// {manifestAddress?'ROBINHOOD MAINNET':'MAINNET LAUNCH PREVIEW'} //</p><img className="game-brand" src="/doom-chain-wordmark.png" alt="DOOM CHAIN"/><h2>ENTER THE<br/><em>EXECUTION ZONE</em></h2><p>Survive the crowd. Scavenge supplies. Reach the exit.</p><button className="primary" onClick={launch} disabled={source==='robinhood'&&!manifestAddress}>{manifestAddress?'▶ PLAY ON ROBINHOOD CHAIN':'▶ PLAY PREVIEW'}</button><small>{manifestAddress?'On-chain cartridge · no wallet needed':'Robinhood mainnet deployment coming soon · no wallet needed'}</small></div>}
     {phase==='loading'&&<div className="boot loading"><div className="loading-brand"><img className="game-brand compact" src="/doom-chain-wordmark.png" alt="DOOM CHAIN"/><p className="eyebrow">BOOT SEQUENCE</p></div><h2>LOADING FROM CHAIN</h2><div className="loading-status"><progress value={progress.percent} max={100}/><p role="status">{progress.label}</p><button className="text-button" onClick={reset}>CANCEL</button></div></div>}
     {phase==='dead'&&<div className="finished death-screen" role="region" aria-label="You died"><img className="game-brand compact" src="/doom-chain-wordmark.png" alt="DOOM CHAIN"/><p className="eyebrow">ROUND OVER</p><h2>YOU DIED</h2><p>The arena claimed another.</p><p className="result-time">{formatTime(elapsed)}</p><button className="primary play-again" onClick={launch}>↻ PLAY AGAIN</button></div>}
     {phase==='complete'&&<div className="finished" role="region" aria-label="Round completed"><img className="game-brand compact" src="/doom-chain-wordmark.png" alt="DOOM CHAIN"/><p className="eyebrow">EXIT REACHED</p><h2 className="executed-title">EXECUTED</h2><p className="result-time">{formatTime(elapsed)}</p>
      {ranked?(saved?<p className="score-success" role="status">TIME SAVED TO THE LEADERBOARD</p>:<form className="score-form" onSubmit={saveScore}><label htmlFor="player-name">YOUR NAME</label><div><input id="player-name" value={name} onChange={e=>setName(e.target.value)} minLength={2} maxLength={20} required autoComplete="nickname" placeholder="Enter your name" disabled={saving}/><button type="submit" disabled={saving}>{saving?'SAVING…':'POST TIME'}</button></div><label htmlFor="player-wallet">PRIZE WALLET ADDRESS</label><input id="player-wallet" value={wallet} onChange={e=>setWallet(e.target.value)} required pattern="0x[0-9a-fA-F]{40}" maxLength={42} placeholder="0x…" autoComplete="off" spellCheck={false} disabled={saving}/><small>Your name and a shortened payout address appear on the leaderboard. The full address is stored for prizes and is not shown publicly. Double-check it before submitting; ownership is not verified.</small>{scoreError&&<p role="alert">{scoreError}</p>}</form>):<p className="ranking-note">Preview rounds do not enter the daily prize leaderboard. Ranked play opens with the Robinhood mainnet launch.</p>}
   ...
   <aside>
    ...
    <p className="source-note">DOOM CHAIN is live on Robinhood Chain mainnet. The browser reconstructs the on-chain cartridge and verifies every byte before play.</p>
  ...
  <footer><span>DOOM CHAIN <b>© 2026</b></span><span>BUILT TO RUN. STORED TO LAST.</span><span>INDEPENDENT / NOT AFFILIATED WITH ID SOFTWARE OR ROBINHOOD</span></footer>
 </main>;
}
