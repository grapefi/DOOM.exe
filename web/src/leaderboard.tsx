...
export function Leaderboard({revision}:{revision:number}){
 ...
 return <section className="leaderboard" id="leaderboard" aria-labelledby="leaderboard-title">
  ...
  <div className="daily-pot" aria-label="Daily prize distribution"><div><strong>DAILY TRADING FEE POT</strong><span>Awarded every day at 00:00 UTC</span></div><ol>{medals.map((medal,i)=><li className={medal.tone} key={medal.tone}><span className={`doom-medal ${medal.tone}`} aria-hidden="true"><img src="/doom-pixel-medal.png" alt=""/></span><b>{i+1}{i===0?'ST':i===1?'ND':'RD'}</b><strong>{medal.share}</strong></li>)}</ol></div>
