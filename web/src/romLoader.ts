...
export async function loadRom(source:CartridgeSource,report:(p:Progress)=>void,signal:AbortSignal){
 ...
 report({label:`Reading ${activeChain.name} manifest`,percent:3});
 ...
 const result=await fetchOnchain(client,address,(n,total)=>report({label:`Reading chunk ${n} / ${total}`,percent:5+Math.round(n/total*75)}),signal,activeChain);
