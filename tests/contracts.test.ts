...
const provider=ganache.provider({chain:{chainId:4663,hardfork:'shanghai'},wallet:{accounts:[{secretKey:key,balance:'0x3635c9adc5dea00000'}]},logging:{quiet:true}});
...
describe('immutable bytecode storage',()=>{
 ...
 it('loader refuses a different chain',async()=>{await expect(fetchOnchain({...client,getChainId:async()=>1} as typeof client,account.address)).rejects.toThrow('4663');});
});
