...
export function roundShareUrl(elapsedMs?:number,name?:string){
 const text=elapsedMs===undefined
  ?'24 hostiles. One way out. Play DOOM.EXE in your browser and challenge the fastest finishers. Can you take the top spot?'
  :name
   ?`${name} escaped DOOM.EXE in ${formatTime(elapsedMs)}. Think you can beat that time? Play the arena and join the leaderboard.`
   :`I escaped DOOM.EXE in ${formatTime(elapsedMs)}. Think you can beat my time? Play the arena and join the leaderboard.`;
 return 'https://twitter.com/intent/tweet?'+new URLSearchParams({text,url:playUrl,hashtags:'DOOMEXE'}).toString();
}
